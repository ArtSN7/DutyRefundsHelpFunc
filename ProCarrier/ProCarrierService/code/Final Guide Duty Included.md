...
├─ Таможня: VAT = 0%
├─ Брокер платит: €0 (только duty если есть)
...
**Box 5: €0** (IOSS не имеет input VAT)
...
**Действия:**
- ✅ Box 5b: Reclaim €87.11 (NL import VAT)
...
- ✅ NL RGR: Reclaim €87.11 VAT + €14.80 Duty
...
- 💰 Commission: (€43.55 + €7.40) × 20% = €10.19
...
**Форма:** Verzoek tot terugbetaling
**Where:** belastingdienst.nl
**When:** Within 3 years (submit soon!)
**Count:** ONE form per returned parcel

**Section 4: Charges Paid - CRITICAL!**
...
**Форма:** Repayment of Import Duty and VAT
**Where:** revenue.ie
**When:** Within 3 years
**Count:** ONE form per returned parcel

**Section 4: Charges Paid - IRELAND SPECIAL!**
...
**Important:** Submitted under YOUR company (NL company):
```
...
## Import VAT (Atlantic/Broker pays):
...
## Sales VAT (You pay via OSS/other):

```
For distance sales:
└─ Parcel Price × Destination VAT Rate
└─ Example: €400 × 19% (Germany) = €76
```
...
```
[ ] Calculate IOSS sales by country
    └─ Pro Carrier VAT amounts

[ ] Calculate IOSS returns by country
    └─ Pro Carrier VAT amounts (negative)

[ ] Fill Dutch VAT Return
    ├─ Box 1a: All IOSS sales
    ├─ Box 1c: All IOSS returns
    └─ Box 5b: ONLY A-NL imports

[ ] Note commissions
    └─ Ireland: 30%, Others: 20%
```
...
```
[ ] Identify A-NL active sales (if any)
    └─ These go in OSS

[ ] Calculate sales by destination country
    └─ Formula: Parcel × destination rate

[ ] Verify: NO IE in OSS
    └─ IE is always domestic!

[ ] Fill OSS Return
    ├─ Per country
    ├─ Include A-NL sales
    ├─ Include C-NL-X returns (negative)
    └─ Net per country
```
...
**КОНЕЦ ФИНАЛЬНОГО РУКОВОДСТВА** ✅

Все правильно:
- VAT = (Parcel + Duty) × Rate
- IOSS без разделения
- IE только domestic
- Правильные комиссии
